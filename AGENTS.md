...
- In Conductor workspaces, `CONDUCTOR_PORT` is the first port in a 10-port range assigned to the workspace; generated app and Postgres ports should stay inside that range.
- Generated `.env` files must not write `WORKTREE_DEV_PORT` or `WORKTREE_POSTGRES_PORT`; those variables are manual overrides only when `CONDUCTOR_PORT` is unset.
- `bun run db:start`, `bun run admin:bootstrap-dev`, and `bun dev` pass derived local env directly; `ports:write` is only for explicitly writing an inspectable `.env`.
